...
LABELS = {
    'ID': {
        ...
        'generate': "BUAT .SII",
        'open_folder': "Buka Folder Tujuan", # <-- DITAMBAHKAN
    },
    'EN': {
        ...
        'open_folder': "Open Destination Folder", # <-- ADDED
    }
}
...
class SCS_PT_sii_generator(bpy.types.Panel):
    ...
    bl_region_type = 'TOOLS' # Sesuai untuk Blender 2.79
    ...
    def draw(self, context):
        ...
        def draw_collection_with_add_remove(layout, props, prop_name, label):
            ...
            op = header.operator("scs.add_sii_item", text="", icon='PLUS', emboss=True) # Icon 'PLUS' untuk 2.79
        ...
        if sii_props.def_type == 'accessory':
            pass
        elif sii_props.def_type == 'cabin' or sii_props.def_type == 'chassis':
            col.separator()
            box = col.box()
        ...
        props = sub.operator('scene.select_directory_inside_base', text="", icon='FILE_FOLDER') # Icon 'FILE_FOLDER' untuk 2.79
        ...
        sub.operator('scs.select_interior_model_path', text="", icon='FILE_FOLDER')
        ...
        # Ikatkan kondisi enabled pada baris ini ke properti saklar
        # Properti ini harus ditambahkan di file definisi PropertyGroup Anda
        row.enabled = sii_props.is_open_folder_enabled

        # Operator tombolnya
        op = row.operator("scs.open_sii_folder", text=L['open_folder'], icon='FILE_FOLDER')

        # Berikan path folder ke operator. os.path.dirname berfungsi di Python 3.5
